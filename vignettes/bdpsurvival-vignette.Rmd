...
$$\lambda_j \sim \mathcal{G}amma\left(a_0+D_j,\,b_0+T_j\right),$$
...
\end{array}
$$
In a future release of the package, we will briefly demonstrate via simulation how well this normal approximation works in practice.
...
$$\lambda_{j} \sim \mathcal{G}amma\left(a_0+\sum_id_{ij},\,b_0+\sum_it_{ij}\right)$$
and

$$\lambda_{j0} \sim \mathcal{G}amma\left(a_0+\sum_id_{0ij},\,b_0+\sum_it_{0ij}\right)$$
respectively. The next steps are dependent on whether a one-arm or two-arm analysis is requested.
...
and
$$p_{S0} = 1-F_p\left(t^\ast,\,\lambda_{10},\dots,\,\lambda_{J0},\,\tau_0,\,\dots\,\,\tau_J\right),$$
be the posterior survival probabilities for the current and historical data, respectively. Then, we compute the posterior probability that the current survival is greater than the historical survival $p = Pr\left(p_S > p_{s0}\right)$. Finally, for a Weibull distribution function (i.e., the Weibull cumulative distribution function), denoted $W$, $\hat{\alpha}$ is computed as 


### Estimation under a two-arm analysis
Under a two-arm analysis, the comparison of interest is the hazard ratio of current vs. historical data. We estimate the log hazard ratio $\beta$ as described previously and compute the posterior probability that $\beta>0$ as $p = Pr\left(\beta > 0\right)$.

***

Now, upon computing the posterior probability $p$, we can compute the historical data weight $\hat{\alpha}$ as 
...
### Estimation under a one-arm analysis
With $\hat{\alpha}$ in hand, we can now estimate the posterior distribution of the survival probability. Using the notation of the previous sections, the posterior distribution is 
$$
\begin{array}{rcl}
p_S & = & 1-F_p\left(t^\ast,\,\lambda_1,\dots,\,\lambda_J,\,\tau_0,\,\dots\,\,\tau_J\right),\\
\\
...
```
In this example, we've simulated current control survival times from an exponential distribution with rate `1/20` and historical control survival times from an exponential distribution with rate `1/20`. With our dataframe constructed, we can now fit the `bdpsurvival` model. Before proceeding, it is worth pointing out that the discount function is applied separately to the treatment and control data. Now, let's carry out the two-arm analysis using default inputs:
```{r}
set.seed(42)
...
The `print` method of a two-arm analysis is largely different than a one-arm analysis (note that with a two-arm analysis, the `summary` method is identical to `print`). First, we see the stochastic comparisons reported for both the treatment and control arms. As previously reported, the stochastic comparison between the current and historical data for the treatment data is relatively small at ``r round(fit2$posterior_treatment$p_hat,4)``, giving a low historical data weight of ``r round(fit2$posterior_treatment$alpha,4)``. On the other hand, the stochastic comparison between the current and historical data for the control data is relatively large at ``r round(fit2$posterior_control$p_hat,4)``, giving a high historical data weight of ``r round(fit2$posterior_control$alpha,4)``. Finally, the presented `coef` value is the log hazard ratio between the augmented treatment data and the augmented control data computed as `log(treatment) - log(control)`.
